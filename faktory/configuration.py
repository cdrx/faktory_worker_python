...
from faktory.utilities.collections import DotDict

DEFAULT_CONFIG = os.path.join(os.path.dirname(__file__), "config.toml")
# User config, for specifying either private defaults or ones that
# need to be guaranteed to overwrite the defaults we provide.
USER_CONFIG = os.getenv("FAKTORY_USER_CONFIG_PATH", "~/.faktory/config.toml")
# Environment variables leading with FAKTORY are parsed and loaded,
# all others are ignored. Key in the environment expected to be in
# the format `FAKTORY__{SECTION}__[...{SUBSECTIONS}]`
ENV_VAR_PREFIX = "FAKTORY"
